...
import GalleryProjects from "@/app/components/gallery-projects";
...
async function GalleryPage(nextParams: { params: { locale: "es" | "de" | "en" } }) {
  const {
    params: { locale },
  } = nextParams;
  const data = await getWordPressCustomPage(locale, "gallery");
  const { acf } = data;
  const { gallery } = acf;
  const t = await getTranslations();

  const galleryData = gallery.map((project) => ({
    image: {
      url: project.url,
      alt: project.alt,
      width: project.width,
  ...
  return (
    <div className="page-aboutUs">
      <GalleryProjects gallery={gallery} />
    </div>
  );
}
